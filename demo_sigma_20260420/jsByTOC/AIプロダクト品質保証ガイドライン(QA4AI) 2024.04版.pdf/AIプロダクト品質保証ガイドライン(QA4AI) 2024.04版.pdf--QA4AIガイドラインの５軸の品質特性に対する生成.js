...
AI プロダクト品質保証ガイドライン
合、特定の目的に対するデータの収集や(ある種の) モデル構築を行うことと言えるが、学習済みの
LLM を前提としていることから、従来の機械学習におけるデータ収集やモデル構築と同様に考える
ことはできない。
...
である。例えば学習出たーの量の十分性に関するチェックリスト(a.ii) では「想定する要求・適用環
...
データを適切に収集する必要がある。
データの量にていては、LLM ではスケーリング則と呼ばれる特性が知られており、データ量が多
...
る。このため、学習に使われたデータが公開されているLLM を利用するとが、LLM を利用したシ
ステムを提供する際のリスクを下げることに繋がる。
学習済みLLM に追加学習する場合
